...
`tools/pgo/pgo.sh` is a standalone script that pieces together the other
...
```sh
cd foot
./tools/pgo/pgo.sh auto . /tmp/foot-pgo-build-output
```

(run `./tools/pgo/pgo.sh` to get help on usage)
...
`tools/generate-alt-random-writes.py`:

```sh
./tools/utils/xtgettcap
./footclient --version
./foot --version
tmp_file=$(mktemp)
../../tools/generate-alt-random-writes \
    --rows=67 \
    --cols=135 \
    --scroll \
    --scroll-region \
...
We will use the script `tools/generate-alt-random-writes.py`:
...
The first step, `./tools/utils/xtgettcap && ./footclient --version`
